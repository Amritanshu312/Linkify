...
const CreateLink = () => {
	...
	const handleSubmit = async () => {
		...
		if (
			userInfo?.linksAllowed > 0 &&
			links?.totalLinks >= userInfo?.linksAllowed
		) {
			return toast.error('Link Creation Exceeded', { position: 'bottom-left' });
		}

		if (
			userInfo?.maxClicks > 0 &&
			userInfo?.totalClicks >= userInfo?.maxClicks
		) {
			...
			(async () => {
				const response = await fetch('/api/user/links/create', {
					...
					body: JSON.stringify({
						...
						expiration:
							expiration !== 'Never' ? convertToISOString(expiration) : null,
					}),
				});
		...
		<AnimatePresence>
			{isCreateLinkPopup && (
				<>
					...
					>
						<div
							className="w-full h-14 flex items-center px-8 border-b-2 border-[#181f3eeb] text-[#ffffff87] cursor-pointer"
						...
						<div className="flex flex-col justify-between h-[92%]">
							<div className="w-full h-full px-6 pt-6">
								...
								<InputItem
									...
									targetValue={originalUrl}
									setTargetValue={setOriginalUrl}
								/>
								<InputItem
									title="Shorten key"
									placeholder="(optional)"
									targetValue={shortenKey}
									setTargetValue={(value) => console.log(value)}
									textBeforeInput
									siderounded
									disabled
								/>
								...
								<div className="mt-8 flex flex-col gap-2">
									<div className="text-[#d2d7df] font-medium text-[15px]">
										Expiration
									</div>
