        ...
        <div className="flex gap-3.5 w-full">
          ...
          <div className="flex gap-2 flex-col">
            <div className="flex gap-3 items-center">
              <Link href={`${process.env.NEXT_PUBLIC_WEBSITE_URL}/${data?.short_url}`} target="_" className="font-medium max-[655px]:text-[15px] max-[510px]:text-[13px]">{process.env.NEXT_PUBLIC_WEBSITE_URL.replace("http://", "").replace("https://", "")}/{data?.short_url}</Link>
            ...
            <div className="flex items-center gap-2">
              ...
              <Link href={data.url} target="_" className="text-[#93a7c5] hover:text-[#a7bfe2] text-[15px] cursor-pointer line-clamp-1 max-[655px]:text-sm max-[940px]:max-w-96 max-[510px]:text-[12px]">{data.url}</Link>
            </div>
          </div>
        </div>

        <div className="max-[585px]:hidden"><BsThreeDots /></div>
      </div>
