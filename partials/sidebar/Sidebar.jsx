	...
	return (
		<motion.div
			...
			className={cn(
				...
				width < 1400 && !isToggled
					? 'w-20'
					: 'max-w-[23rem] w-full overflow-y-scroll [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]'
			)}
		>
			<motion.div
			...
			<div className="flex flex-col justify-between h-full">
				...
				>
					<motion.div variants={itemVariants}>
						<LinkItem
							title={!isToggled && width < 1400 ? '' : 'Links'}
							logo={<PiLinkSimpleBold size={18} />}
							href="/"
							active={pathname === '/'}
						/>
					</motion.div>
					<motion.div variants={itemVariants}>
						<LinkItem
							title={!isToggled && width < 1400 ? '' : 'Analytics'}
							logo={<TbBrandGoogleAnalytics size={18} />}
							href="/analytics"
					...
					<motion.div variants={itemVariants}>
						<LinkItem
							title={!isToggled && width < 1400 ? '' : 'Events'}
							logo={<TbTimelineEvent size={18} />}
							href="/events"
						/>
					</motion.div>
					<motion.div variants={itemVariants}>
						<LinkItem
							title={!isToggled && width < 1400 ? '' : 'Domains'}
							logo={<TbWorldWww size={18} />}
							href="/domains"
						/>
					</motion.div>
				</motion.div>
				...
				>
					...
					<motion.div variants={itemVariants}>
						<LinkItem
							title={!isToggled && width < 1400 ? '' : 'Support'}
							logo={<HiOutlineChatBubbleLeftRight size={19} />}
							href="/support"
						/>
					</motion.div>
